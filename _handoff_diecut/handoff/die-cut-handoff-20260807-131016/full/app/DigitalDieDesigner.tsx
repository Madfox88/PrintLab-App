...
function StatusIcon({ status }: { status: "valid" | "invalid" | "requiresVerification" }) {
  return <span className={`status-icon ${status}`} aria-hidden>{status === "valid" ? "✓" : status === "invalid" ? "×" : "!"}</span>;
}
...
function TechnicalVisualizer({ die, press, calc, layers, webWidthMm }: { die: DieSpecification; press: PressProfile; calc: ReturnType<typeof calculateLayout>; layers: Layers; webWidthMm?: number }) {
  ...
  const height = Math.max(calc.plateRepeatMm + Math.max(calc.leadingOffsetMm, 20) * 2, 260);
  const padding = 42;
  ...
  const plateStartY = Math.max((height - calc.plateRepeatMm) / 2, 0);
  ...
  return <div className="visualizer-shell">
    ...
    <svg className="die-svg" viewBox={`${-padding} ${-padding} ${viewW} ${viewH}`} role="img" aria-label={`Proportional die layout with ${die.layout.labelsAcross} labels across and ${die.layout.labelsAround} around`}
      ...
      <g transform={`translate(${pan.x} ${pan.y}) scale(${zoom})`}>
        ...
        {layers.margins && <rect x={startX} y={labelStartY} width={calc.occupiedWidthMm} height={calc.occupiedLabelLengthMm} className="margin-boundary" />}
    ...
    <div className="legend"><span><i className="blue" />Printable area</span><span><i className="purple" />Cutting repeat</span><span><i className="amber" />10 mm plate margin all around</span><span><i className="black" />Cut path</span><span><i className="red" />Layout boundary</span></div>
  </div>;
}
...
export default function DigitalDieDesigner({ input, callbacks }: DigitalDieDesignerProps = {}) {
  ...
  const productionStatusTitle = purchase.status === "invalid" ? "Geometry invalid"
    : purchase.status === "requiresMachineVerification" ? "Requires machine verification"
      : purchase.status === "requiresSupplierReview" ? "Requires supplier review"
  ...
  }, [callbacks, effectiveDie, calc, purchase, dirty, purchasePolicy.blocked]);
  ...
  return <main className="app-shell">
    <header className="topbar">
      <div className="brand-mark">DD</div><div className="brand-copy"><h1>Digital Die Designer</h1><p>Production planning · HP Indigo & GM finishing</p></div>
    ...
    {tab === "calculator" && <div className="workspace-grid">
      ...
      <section className="visual-panel" aria-label="Technical visualizer">
        <div className="panel-heading"><div><span className="eyebrow">TECHNICAL VIEW</span><h2>Die layout · proportional scale</h2></div><div className="layer-menu">{Object.entries(layers).map(([key, shown]) => <label key={key}><input type="checkbox" checked={shown} onChange={(e) => setLayers((current) => ({ ...current, [key]: e.target.checked }))} />{key}</label>)}</div></div>
        <TechnicalVisualizer die={active} press={press} calc={calc} layers={layers} webWidthMm={previewWebWidthMm} />
      ...
      <aside className="results-panel" aria-label="Calculation results">
        <div className={`fit-banner ${calc.status}`} aria-live="polite" aria-atomic="true"><StatusIcon status={calc.status} /><div><span>{productionStatusTitle}</span><small>{calc.errors.length ? `${calc.errors.length} blocking geometry issue${calc.errors.length > 1 ? "s" : ""}` : incompleteRequiredCount ? `${incompleteRequiredCount} required specification check${incompleteRequiredCount > 1 ? "s" : ""} incomplete` : "All required specification checks are complete"}</small></div></div>
    ...
    {tab === "purchase" && <PurchaseView active={active} purchase={purchase} policy={purchasePolicy} />}
    {toast && <div className="toast" role="status">✓ {toast}</div>}
  </main>;
}

function Library({ state, active, setState, setTab, setToast, importRef }: { state: SavedState; active: DieSpecification; setState: React.Dispatch<React.SetStateAction<SavedState>>; setTab: (tab: Tab) => void; setToast: (value: string) => void; importRef: React.RefObject<HTMLInputElement | null> }) {
...
function PurchaseView({ active, purchase, policy }: { active: DieSpecification; purchase: DiePurchaseSpecification; policy: ReturnType<typeof getPurchaseExportPolicy> }) {
  ...
  return <section className="page-view purchase-page"><div className="page-title"><div><span className="eyebrow">HUMAN-REVIEWED SUPPLIER REQUEST</span><h2>Die purchase specification</h2><p>The app prepares this document; a person reviews and sends it manually.</p></div><div className="button-row"><button className="secondary" disabled={!policy.copy} title={blockedTitle} onClick={() => navigator.clipboard.writeText(text)}>Copy purchase specification</button><button className="secondary" disabled={!policy.draftCsv} title={blockedTitle} onClick={() => download(`${active.id}-purchase.csv`, exportPurchaseCsv(purchase), "text/csv")}>{policy.draft ? "Export draft CSV" : "Export CSV"}</button><button className="secondary" disabled={!policy.draftJson} title={blockedTitle} onClick={() => download(`${active.id}-purchase.json`, json)}>{policy.draft ? "Export draft JSON" : "Export JSON"}</button><button className="primary" disabled={!policy.print} title={blockedTitle} onClick={() => window.print()}>Print specification</button></div></div>
